...
3. **Listen** — Scrape each **hot** prospect's public footprint:
   - **LinkedIn** via Apify (harvestapi actors): profile, their posts, and their activity feed — the comments + reactions they *leave* (public on LinkedIn; this is the "content they like" layer), including the **media types** they engage with (text, image, document/carousel, video, poll — post types are in the scraped data)
...
4. **Model** — One Claude subagent per prospect, in parallel, each emitting a structured **Buyer World Model** (schema in §6): topics, formats they reward, influencers in their orbit, stances, what they post vs. only consume — every claim tied to evidence URLs. A synthesis pass merges them into the **Gravity Map**: the shared conversation your ICP is already having, and where it happens. World models then cluster into **taste cohorts** — buyers who reward the same formats and topics (chart skeptics, systems thinkers, quiet execs) — so one piece of content serves a whole cohort, never just one person.
5. **Create** — Claude generates a 5-day **gravity plan** from the world models, the Gravity Map, and Sillage's signal matches + generated briefs (`get_contents`) — three action types, **each aimed at a taste cohort**:
   - **Posts** (LinkedIn + X, format-tuned, **in your voice** — tone profile read from your own posts — 2 variants each to A/B, and **editable**: comment on any draft ("mention our 6.4% reply rate") → Strategist revises with your note, and the note becomes a permanent brain rule): *"6 of your 10 targets engaged with tactical charts about SDR productivity this month — this post is that format, on that topic, tied to your product narrative."*
...
| **Strategist** | 5 | Gravity Map → **taste cohorts** → the week's plan: posts, comments, micro-actions, one cohort per action |
...
**App:** Next.js dashboard — onboard (paste your site) → prospect board (heat + Gravity Score ring + state) → world model view → gravity plan → warm queue. Backend: TypeScript; the crew is defined as **Claude managed agents** (Agent SDK `options.agents`, headless: `settingSources: []`, `permissionMode: "dontAsk"`, structured outputs via `outputFormat: json_schema`), built on the **goals → loops → evals** pattern: every agent prompt carries an explicit goal and self-eval rubric; create↔measure runs as a standing loop; every draft passes an **eval gate** (their format · our voice · evidence-backed · zero slop) and is auto-revised until it clears. Everything learned lands in the **Company Brain** (`data/runtime/brain.json`, persists across runs): narrative, ICP, **tone of voice read from your own posts**, content performance, user steering notes, and decisions-with-reasons — every generation reads the digest, so decisions get better with every piece of data. Crew defined with **both sponsor MCPs** — Sillage (`api.getsillage.com/api/mcp/v2`, OAuth workspace login, ~35 tools) and FullEnrich (`mcp.fullenrich.com/mcp`) — plus 6 custom tools (`read_website`, `hubspot_pipe`, `apify_run`, `x_api`, `x_search`, `gradium_stt`). Install Sillage's own playbooks (`npx skills add sillage-labs/skills`) so the agent drives the workspace their way: persona → accounts → coverage → agents → runs. Drop to REST (FullEnrich webhooks, Sillage `sk_live_`) only where a flow needs it. Models: `claude-sonnet-5` for the parallel prospect modelers; **`claude-fable-5` writes every human-facing word** — posts, comments, outreach, pitch briefs, gravity-map synthesis — under an explicit anti-slop contract in the prompts (casual, specific, zero AI tells) enforced by the eval gate's banned-phrase list. Storage: SQLite/JSON — hackathon-grade.
...
// produced by Claude as structured output
{
  ...
  "media": [             // the MEDIUM they engage with
